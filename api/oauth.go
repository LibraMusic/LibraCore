	...
	providerFactories = map[string]providerFactory{
		...
		"cognito": func(p OAuthProvider) (goth.Provider, error) {
			...
			if authURL == "" || tokenURL == "" || issuerURL == "" || profileURL == "" {
				return nil, fmt.Errorf("missing base_url or auth_url, token_url, issuer_url, or profile_url for cognito provider")
			}
		...
		"github": func(p OAuthProvider) (goth.Provider, error) {
			authURL := p.AdditionalDetails["auth_url"]
			tokenURL := p.AdditionalDetails["token_url"]
			profileURL := p.AdditionalDetails["profile_url"]
			...
			if authURL != "" || tokenURL != "" || profileURL != "" || emailURL != "" {
				if authURL == "" || tokenURL == "" || profileURL == "" || emailURL == "" {
					return nil, fmt.Errorf("missing auth_url, token_url, profile_url, or email_url for github provider")
				}
		...
		"gitlab": func(p OAuthProvider) (goth.Provider, error) {
			authURL := p.AdditionalDetails["auth_url"]
			tokenURL := p.AdditionalDetails["token_url"]
			profileURL := p.AdditionalDetails["profile_url"]
			if authURL != "" || tokenURL != "" || profileURL != "" {
				if authURL == "" || tokenURL == "" || profileURL == "" {
					return nil, fmt.Errorf("missing auth_url, token_url, or profile_url for gitlab provider")
				}
		...
		"nextcloud": func(p OAuthProvider) (goth.Provider, error) {
			...
			profileURL := p.AdditionalDetails["profile_url"]
			if authURL == "" || tokenURL == "" || profileURL == "" {
				return nil, fmt.Errorf("missing nextcloud_url or auth_url, token_url, or profile_url for nextcloud provider")
			}
		...
		"okta": func(p OAuthProvider) (goth.Provider, error) {
			...
			profileURL := p.AdditionalDetails["profile_url"]
			if authURL == "" || tokenURL == "" || issuerURL == "" || profileURL == "" {
				return nil, fmt.Errorf("missing org_url or auth_url, token_url, issuer_url, or profile_url for okta provider")
			}
		...
		"onedrive": func(p OAuthProvider) (goth.Provider, error) {
			return onedrive.New(p.Key, p.Secret, fmt.Sprintf("%s/auth/onedrive/callback", publicURL)), nil
		},
		"openid-connect": func(p OAuthProvider) (goth.Provider, error) {
		...
		"oura": func(p OAuthProvider) (goth.Provider, error) {
			return oura.New(p.Key, p.Secret, fmt.Sprintf("%s/auth/oura/callback", publicURL), oura.ScopeEmail, oura.ScopePersonal), nil
		},
		"patreon": func(p OAuthProvider) (goth.Provider, error) {
			authURL := p.AdditionalDetails["auth_url"]
			tokenURL := p.AdditionalDetails["token_url"]
			profileURL := p.AdditionalDetails["profile_url"]
			if authURL != "" || tokenURL != "" || profileURL != "" {
				if authURL == "" || tokenURL == "" || profileURL == "" {
					return nil, fmt.Errorf("missing auth_url, token_url, or profile_url for patreon provider")
				}
		...
		"paypal": func(p OAuthProvider) (goth.Provider, error) {
			authURL := p.AdditionalDetails["auth_url"]
			tokenURL := p.AdditionalDetails["token_url"]
			profileURL := p.AdditionalDetails["profile_url"]
			if authURL != "" || tokenURL != "" || profileURL != "" {
				if authURL == "" || tokenURL == "" || profileURL == "" {
					return nil, fmt.Errorf("missing auth_url, token_url, or profile_url for paypal provider")
				}
		...
		"tumblr": func(p OAuthProvider) (goth.Provider, error) {
			...
			if useAuthorizeString != "" {
				...
				if err != nil {
					return nil, fmt.Errorf("invalid use_authorize value for tumblr provider")
				}
				if useAuthorize {
		...
		"twitch": func(p OAuthProvider) (goth.Provider, error) {
			return twitch.New(p.Key, p.Secret, fmt.Sprintf("%s/auth/twitch/callback", publicURL)), nil
		},
		"twitter": func(p OAuthProvider) (goth.Provider, error) {
			useAuthorizeString := p.AdditionalDetails["use_authorize"]
			if useAuthorizeString != "" {
				useAuthorize, err := strconv.ParseBool(useAuthorizeString)
				if err != nil {
					return nil, fmt.Errorf("invalid use_authorize value for twitter provider")
				}
				if useAuthorize {
