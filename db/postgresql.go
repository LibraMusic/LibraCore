...
func (db *PostgreSQLDatabase) Connect() (err error) {
	...
	_, err = db.sqlDB.Query("CREATE EXTENSION IF NOT EXISTS hstore;")
	if err != nil {
		return
	}
...
func (db PostgreSQLDatabase) GetTrack(id string) (types.Track, error) {
	track := types.Track{}
	row := db.sqlDB.QueryRow("SELECT * FROM tracks WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetTracks(userID string) ([]types.Track, error) {
	...
	rows, err := db.sqlDB.Query("SELECT * FROM tracks WHERE user_id=$1;", userID)
	if err != nil {
		return tracks, err
	}
	defer rows.Close()
...
func (db PostgreSQLDatabase) GetAlbum(id string) (types.Album, error) {
	album := types.Album{}
	row := db.sqlDB.QueryRow("SELECT * FROM albums WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetAlbums(userID string) ([]types.Album, error) {
	...
	rows, err := db.sqlDB.Query("SELECT * FROM albums WHERE user_id=$1;", userID)
	if err != nil {
		return albums, err
	}
	defer rows.Close()
...
func (db PostgreSQLDatabase) GetVideo(id string) (types.Video, error) {
	video := types.Video{}
	row := db.sqlDB.QueryRow("SELECT * FROM videos WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetVideos(userID string) ([]types.Video, error) {
	...
	rows, err := db.sqlDB.Query("SELECT * FROM videos WHERE user_id=$1;", userID)
	if err != nil {
		return videos, err
	}
	defer rows.Close()
...
func (db PostgreSQLDatabase) GetArtist(id string) (types.Artist, error) {
	artist := types.Artist{}
	row := db.sqlDB.QueryRow("SELECT * FROM artists WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetArtists(userID string) ([]types.Artist, error) {
	...
	rows, err := db.sqlDB.Query("SELECT * FROM artists WHERE user_id=$1;", userID)
	if err != nil {
		return artists, err
	}
	defer rows.Close()
...
func (db PostgreSQLDatabase) GetPlaylist(id string) (types.Playlist, error) {
	...
	row := db.sqlDB.QueryRow("SELECT * FROM playlists WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetPlaylists(userID string) ([]types.Playlist, error) {
	...
	rows, err := db.sqlDB.Query("SELECT * FROM playlists WHERE user_id=$1;", userID)
	if err != nil {
...
func (db PostgreSQLDatabase) UsernameExists(username string) (bool, error) {
	var exists bool
	err := db.sqlDB.QueryRow("SELECT EXISTS(SELECT 1 FROM users WHERE username=$1);", strings.ToLower(username)).Scan(&exists)
	return exists, err
}

func (db PostgreSQLDatabase) EmailExists(email string) (bool, error) {
	var exists bool
	err := db.sqlDB.QueryRow("SELECT EXISTS(SELECT 1 FROM users WHERE email=$1);", strings.ToLower(email)).Scan(&exists)
	return exists, err
}

func (db PostgreSQLDatabase) GetUser(id string) (types.User, error) {
	user := types.User{}
	row := db.sqlDB.QueryRow("SELECT * FROM users WHERE id=$1;", id)
...
func (db PostgreSQLDatabase) GetUserByUsername(username string) (types.User, error) {
	user := types.User{}
	row := db.sqlDB.QueryRow("SELECT * FROM users WHERE username=$1 OR email=$1;", strings.ToLower(username))
	err := row.Scan(&user.ID, &user.Username, &user.Email, &user.PasswordHash, &user.DisplayName, &user.Description, &user.ListenedTo, &user.Favorites, &user.PublicViewCount, &user.CreationDate, &user.Permissions, &user.LinkedArtistID, &user.LinkedSources)
	return user, err
}
...
func (db PostgreSQLDatabase) BlacklistToken(token string, expiration time.Time) error {
	_, err := db.sqlDB.Exec("INSERT INTO blacklisted_tokens (token, expiration) VALUES ($1, $2);", token, expiration)
	return err
}

func (db PostgreSQLDatabase) CleanExpiredTokens() error {
	_, err := db.sqlDB.Exec("DELETE FROM blacklisted_tokens WHERE expiration < NOW();")
	return err
}

func (db PostgreSQLDatabase) IsTokenBlacklisted(token string) (bool, error) {
	var exists bool
	err := db.sqlDB.QueryRow("SELECT EXISTS(SELECT 1 FROM blacklisted_tokens WHERE token=$1);", token).Scan(&exists)
	return exists, err
}
